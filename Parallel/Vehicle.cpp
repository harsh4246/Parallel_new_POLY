...
void Vehicle :: T_interaction()
        {
            for(int j=0;j<this->targetsSensed.size();j++){
                ...
                double distance;
                for (int i=0;i<3;i++){
                    distanceVector[i]=this->position[i]-this->targetsSensed[j][i];
                }
...
void Vehicle:: O_interaction()
        {
            for(int j=0;j<this->obstaclesSensed.size();j++){
                ...
                double distance;
                for (int i=0;i<3;i++){
                    distanceVector[i]=this->position[i]-this->obstaclesSensed[j][i];
                }
                distance=this->magnitudeOfVector(distanceVector);
                for (int i=0;i<3;i++){
                    distanceVector[i]=distanceVector[i]/distance;
...
void Vehicle:: M_interaction()
        {
            for(int j=0;j<this->membersSensed.size();j++){
                ...
                double distance;
                for (int i=0;i<3;i++){
                    distanceVector[i]=this->position[i]-this->membersSensed[j][i];
                }
                distance=this->magnitudeOfVector(distanceVector);
                if(distance==0.0){
                    continue;
